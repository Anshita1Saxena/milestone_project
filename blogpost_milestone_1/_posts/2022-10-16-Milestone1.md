...
In this section, we will build a function named "download_raw_data" to download all NHL data of a season based on the provided target season/ year and a "dir_path".
...
GAME_ID contains 10 digits in total, where the first 4 digits indicate the season of the game (e.g., 2018), while the next 2 digits indicate type of game (01 = preseason, 02 = regular season, 03 = playoffs, 04 = all-star). The final 4 digits identify a specific game in the season. For regular games, this ranges from 0001 to 1271 (for seasons with 31 teams) or to 1230 (for those have 30 teams). For playoff games, the last 3 digits account for round, matchup and game number respectively. 
...
Then, the function downloads data of all REGULAR games within the target season. From season 2017 to 2020, there were 31 teams with 1271 games in each season. However, only 30 teams involved in season 2016, therefore the total number of games is 1230 in that year.
...
Similarly, data of playoff games are downloaded based on its game ID. It is noticed that there are 4 playoff rounds for each season. The first round starts with 8 matchups, then the number of matchups is halved for each following round. Besides, each matchup includes 7 games in total.
...
You can run the code multiple times if you want to ensure that all data are there. Existing files will not be skipped automatically by the function so that it will not waste your time!
...
- When a player in a team gets penalty, the count value of that team is substracted by 1.
- If the penalty time is over or the enemy team has a goal, the subtracted count is recovered.
...
- Firstly, we can calculate and add the distance of the shot to the goal. This is an useful information, partly indicating how dangerous each shot is, or showing the strategy of each team/ player.
- Secondly, we could include the "rebound" information by taking into account the previous event of each shot. If the previous event is a blocked shot, this means the current shot is a rebound shot.
- Finally, shots during the penalty time can also be added into our dataframe. It might show how lack of players affects each team's defense.
