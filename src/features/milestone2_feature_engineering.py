...
def calc_shot_distance(x_shot, y_shot, rink_side):
    ...
    coor_shot = np.array([x_shot, y_shot])
...
def calc_shot_angle(x_shot, y_shot, rink_side):
    ...
    x_dist_abs = np.abs(x_net - x_shot)
    y_dist_abs = np.abs(y_shot)
...
# Run test for the tidied dataframe from the milestone 1 tidydata
df_game = pd.read_csv("df_tidy_data.csv")
df_game = feature_engineering(df_game)
df_game.to_csv('df_feature_engineering.csv',index=False)
