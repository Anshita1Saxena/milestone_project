...
def game_events_to_np(file_path):
    ...
    for play_indx, play in enumerate(all_plays):
        ...
        if len(data_dict['liveData']['linescore']['periods']) > 0 and period <= 4:
            print(len(data_dict['liveData']['linescore']['periods']))
            print(data_dict['liveData']['linescore']['periods'][0])
            print(period)
    ...
    #     if(np.sum(np_game[:,-1])<np.sum(np_game[:,-2])):
    #         np_game[:,-2] = np_game[:,-1]
...
df_game_events.to_csv('complex_visuals.csv', index=False)
df_game_events
